...
성능 변경·측정 결과·필드/페이지 계약은 [조회 API](../docs/read-api-v2.md)에 있습니다. 백엔드 자동 reload가 반영되지 않아 새 경로가 404이면 API를 재시작합니다.
...
상위 프로젝트의 `.env.catalog`에 있는 `NEW_DATABASE_URL`이 사용자 조회 API와 신규 runtime의 DB 설정입니다. 배포 전환 전까지 기존 `.env`의 `DATABASE_URL`은 읽기 전용 이전 원본으로만 보존합니다. 프론트에서 자동으로 읽거나 복사하지 않습니다. DB 주소는 HTTP API 주소로 사용할 수 없습니다.
