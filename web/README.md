...
상위 프로젝트의 `.env`에 있는 `DATABASE_URL`이 사용자 조회 API와 신규 runtime의 단일 DB 설정입니다. 이전 도구의 구 DB 주소는 `LEGACY_DATABASE_URL`로 별도 지정합니다. 프론트에서 이 값을 자동으로 읽거나 복사하지 않습니다. DB 주소는 HTTP API 주소로 사용할 수 없습니다.
