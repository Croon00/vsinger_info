...
test('artist statistics paginate, retain Enter searches, filter aliases and paginate all artists independently', async ({
  ...
  await expect(page.getByRole('tab')).toHaveText([/라이브/, '통계', '발매곡', '공연 정보'])
  ...
  const headers = songSection.locator('.statistics-page:not([inert]) .statistics-table thead th')
  await expect(headers).toHaveText(['순위', '곡', '최근 부른 날', '횟수'])
  await expect(artistSection.getByRole('heading', { name: 'ヨルシカ (요루시카)' })).toBeVisible()
  for (const column of [0, 2, 3]) {
    await expect(headers.nth(column)).toHaveCSS('text-align', 'center')
    await expect(rows.first().locator('td').nth(column)).toHaveCSS('text-align', 'center')
  }
  await expect(headers.nth(1)).toHaveCSS('text-align', 'left')
  const search = page.getByRole('textbox', { name: '곡명 또는 원곡 아티스트' })
  await search.fill('사양')
  await expect(songSection.getByText('斜陽 (사양)', { exact: true })).toBeVisible()
  await expect(songSection.getByText('ヨルシカ (요루시카)', { exact: true })).toBeVisible()
  await search.fill('')
