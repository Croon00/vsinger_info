...
이 문서는 2026-09-21 단계 종료 당시의 도구·dry-run 기록이다. 현재 도구 계약과 실제 운영 DB 적용 결과는 [2026-09-23 이전 기록](backend-cutover-2026-09-23.md)을 따른다.

완료일: 2026-09-21. 이 단계에서는 기존 DB와 신규 DB를 모두 읽기 전용으로 확인했다. 실제 운영 데이터 이전은 실행하지 않았다.

아래 변수명과 YouTube 선택 범위는 2026-09-21 당시 기록이다. 현재 운영 적용 결과는 상단의 이전 기록을 따른다.
