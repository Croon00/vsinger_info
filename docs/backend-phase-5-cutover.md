...
Railway의 `NEW_DATABASE_URL`은 신규 DB를 가리켜야 한다. 선택적 `NEW_DATABASE_INSTANCE_ID`는 같은 DB의 `SELECT id FROM public.catalog_instance` 결과를 설정해 잘못된 대상 연결을 추가로 차단한다. 값을 설정하지 않아도 schema version과 migration revision 검사는 유지된다. 기존 `NEW_CATALOG_INSTANCE_ID` 이름은 사용하지 않는다.
