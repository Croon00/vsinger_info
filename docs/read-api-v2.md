...
| GET /artists/{id}/statistics | 가창자 ID에 따른 곡·원곡 아티스트 순위와 월별 활동량. 곡의 `titleKo`·`artistKo`, 원곡 아티스트 순위의 `nameKo`는 저장된 한국어 이름이며 없으면 null |
...
- song_id가 없는 세트리스트도 raw_artist의 정확한 표기가 등록 아티스트의 이름·별칭과 같으면 그 아티스트의 다른 이름·별칭으로 검색한다. 이 조회는 song_id나 원곡자 소유권을 자동 등록하지 않는다.
- 검색은 이름·별칭에 맞는 아티스트를 한 번 찾고, 제목·원문·가창자·원곡자 조건에 맞는 세트리스트 ID를 페이지로 자른 뒤 표시 필드를 조회한다. `total`과 정렬 기준은 전체 일치 결과를 기준으로 유지한다.
...
2026-09-24: 임시 로컬 PostgreSQL fixture로 song_id가 없는 `ヨルシカ` 원문을 `요루시카`·영문명·별칭으로 검색하고, 가창 아티스트 이름 검색·페이지 total/순서·기존 조회 경로를 검증했다. 실 DB 읽기 전용 조회에서 `요루시카` 검색은 5회 중간값 약 176ms(50건 반환, total 4166), 아티스트 목록은 약 152ms였다. 이 수치는 직접 DB 호출 시간이며 브라우저 전체 로딩 시간은 아니다. DB 변경은 수행하지 않았다.
