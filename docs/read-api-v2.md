...
프로젝트 루트의 `.env.catalog` 또는 환경변수 `NEW_CATALOG_DATABASE_URL`을 사용한다. 관리자 웹과 같은 DB 설정이며, 접속 문자열은 브라우저로 보내지 않는다. 설정이 없거나 연결이 실패하면 503을 반환하고 기존 DB 또는 목업으로 대체하지 않는다.
