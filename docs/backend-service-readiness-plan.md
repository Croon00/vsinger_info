...
**현재 판정: 로컬 보완 A~F 완료, 전체 서비스 운영 전환 미완료.** 신규 조회 API, X·Discord, 독립 YouTube·등록 Spotify 수집, 선택 이전·설정 경계, 이전→수집→실제 `/api` 통합을 로컬에서 검증했다. 운영 DB 이전·Railway 설정·실제 provider/Discord/배포 웹 검증은 남아 있다. 신규 DB 주소를 설정하고 runtime을 켜는 것만으로 확정 서비스 전체가 정상 동작하지 않는다.

이번 검증은 기존·신규 운영 DB 변경, 실제 provider 조회, Discord 전송, PR 변경, Railway 배포 없이 진행했다. 실제 Railway 배포 commit·환경변수·네트워크·봇 권한은 확인하지 못했다. 아래에서 **테스트 재현**, **코드 확인**, **운영 미검증**을 구분한다.
...
| X 원문 수집 | 보완 1단계에서 기준선·계정 순환·재활성화·lease 수정 | 실제 이전 이후 운영 검증 |
...
6. 로컬 합격 후 [5단계 전환 절차](backend-phase-5-cutover.md)에 따라 writer 정지→최종 snapshot/manifest→선택 이전→읽기 검증→봇 연결→수집/전송 단계적 활성화를 수행한다. 실제 provider·Discord 검증은 이 운영 절차에서 수행하며 로컬 mock 성공으로 대체하지 않는다.
