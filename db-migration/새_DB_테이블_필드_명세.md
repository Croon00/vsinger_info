...
**이 문서는 새로 만들 DB의 상세 설계 제안입니다. 실제 테이블 생성·데이터 적재·서비스 DB 전환을 실행한 결과가 아닙니다.**

통합 아티스트 명부와 음악 카탈로그 우선 이전은 사용자 결정으로 확정했습니다. 세부 타입·기본값·상태 값·고유 제약은 구현 전 검토할 제안입니다. 초기 데이터는 사용자가 전체 반영 대상을 검수한 뒤에만 입력합니다.
...
- 원형 FK는 catalog_imports 생성 후 추가하는 등 migration 순서를 설계합니다. 기술용 migration 버전 테이블은 도구 선택 후 별도 생성합니다.
...
| 도구가 만드는 migration 버전 표 | 도구 선택·구현 단계에서 기술 문서화 |
| 모든 컬럼의 최종 DDL·인덱스·트리거·API 스키마 | 이 상세안을 검토한 뒤 구현하고 검증 |

이 문서는 현재 계획을 이해하고 검토할 수 있게 모든 업무 테이블과 필드를 풀어 쓴 것입니다. 새 DB에 DDL을 실행하거나 초기 데이터를 입력하는 작업을 수행하지 않았습니다.
