...
**2026-09-19 원격 PostgreSQL의 31개 업무 테이블·276개 컬럼 생성 완료. 기술용 마이그레이션 표를 포함하면 32개입니다. 초기 음악 데이터는 0건이며 기존 앱 연결은 유지했습니다. 2026-09-20 로컬 검수 9개 표와 관리자 API/웹을 구현했습니다. 사용자 조회 API의 새 DB 전환은 후속 작업입니다.**

실행 DDL과 DB가 강제하는 규칙/향후 API 검증의 구분은 [마이그레이션 안내](../migrations/catalog/README.md), 실제 확인 값은 [적용 보고서](schema-application-report.json)를 참고하세요.

통합 아티스트 명부와 음악 카탈로그 우선 이전은 사용자 결정으로 확정했습니다. 원격 컬럼·타입·기본값·DB 제약은 migration 001에 반영했습니다. 서비스 검증과 로컬 검수 저장소의 상세안은 아직 구현 전입니다. 초기 데이터는 사용자가 전체 반영 대상을 검수한 뒤에만 입력합니다.
