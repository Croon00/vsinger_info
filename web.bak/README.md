...
2026-09-19 기존 web 디렉터리를 web.bak으로 이동했다. 현재 기본 프론트는 [web](../web/README.md)이다. 이 폴더는 기존 코드 보관과 필요 시 관리 기능 실행을 위해 유지한다.

Vue 3 + TypeScript + Vite 기반의 기존 관리 프론트다. Pinia, TanStack Query for Vue, Nuxt UI를 사용한다. 새 사용자 조회 화면은 [web](../web/README.md)에서 별도로 개발한다. 기존 웹은 폐기되지 않았으며 관리·수집 API와의 호환성을 유지한다.
